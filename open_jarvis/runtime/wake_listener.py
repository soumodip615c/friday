...
# How long JARVIS stays active after the wake word.
ACTIVE_TIMEOUT = int(
    os.getenv("JARVIS_ACTIVE_TIMEOUT", "60")
)

# Your working microphone is device #1.
...
def listen_for_wake_word(*, logger, send_log) -> None:
    """Listen continuously for the JARVIS wake word."""
    ...
    while True:
        try:
            # Use the microphone that we already confirmed works.
            with sr.Microphone(
            ...
            if wake_word_detected(
                ...
                send_log(
                    "[VOICE] JARVIS active - continuous conversation enabled"
                )
                ...
                record_runtime_event(
                    ...
                    "info",
                    {
                        ...
                        "microphone_device": MICROPHONE_DEVICE_INDEX,
                    },
                )

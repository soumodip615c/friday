"""Voice personality helpers for FRIDAY."""
...
GOODBYES = [
    "It's been a pleasure serving you today, sir. Friday signing off.",
    ...
    "Goodbye, sir. It was an honour, as always. Friday shutting down.",
...
def greet(
    *,
    ...
    """Speak the initial FRIDAY greeting and push it to the UI log."""
    ...
    safe_speak(
        f"{greeting}, sir. Friday is online. "
        "Say Friday to activate me.",
        speak=speak,
        send_log=send_log,
        logger=logger,
    )
    ...
    # UI log
    send_log(
        f"FRIDAY: {greeting}, sir. All systems are ready."
    )
...
def say_goodbye(
    *,
    ...
    if 22 <= hour or hour < 6:
        safe_speak(
            random.choice(GOODBYES) + " Get some rest, sir.",
            speak=speak,
            send_log=send_log,
            logger=logger,
        )
...
def maybe_tell_joke(
    *,
    ...
    state["command_count"] = (
        state.get("command_count", 0) + 1
    )

    if state["command_count"] >= state.get(
        "joke_interval",
        5,
    ):
        state["command_count"] = 0

        state["joke_interval"] = random.randint(
            5,
            8,
        )

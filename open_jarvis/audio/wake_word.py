...
def build_wake_word_config(
    ...
    voice_enabled = parse_bool(
        source.get("JARVIS_VOICE_ENABLED"),
        True,
    )

    enabled = (
        parse_bool(
            source.get("JARVIS_WAKE_WORD_ENABLED"),
            True,
        )
        and voice_enabled
    )
    ...
    try:
        cooldown_seconds = max(
            ...
            float(
                source.get(
                    "JARVIS_WAKE_WORD_COOLDOWN_SECONDS",
                    "1.0",
                )
            ),
        )
...
def _config_value(
    config: WakeWordConfig | Mapping[str, object] | None,
    key: str,
    default: object,
) -> object:
    if config is None:
...
def analyze_wake_word(
    ...
    *,
    ...
    if (
        ...
        and now is not None
        and now - last_detected_at < cooldown_seconds
    ):
    ...
    if (
        wake_tokens
        and len(tokens) >= len(wake_tokens)
        and not cooldown_active
    ):
        detected = any(
            tokens[index : index + len(wake_tokens)]
            == wake_tokens
            for index in range(
                len(tokens) - len(wake_tokens) + 1
            )
        )
    ...
    def __init__(
        ...
        *,
        ...
        self.config = WakeWordConfig(
            wake_word=(
                normalize_voice_phrase(wake_word)
                or "friday"
            ),
            enabled=enabled,
            cooldown_seconds=max(
